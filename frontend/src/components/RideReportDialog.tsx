...
import type { ReportReasonCode, ReportReasonOption } from "@/lib/rideReports";
import { useState } from "react";

export function RideReportDialog<TCode extends ReportReasonCode>({
  confirmLabel = "Submit Report",
  description,
  detailPlaceholder = "Describe what happened...",
  isOpen,
  isSubmitting,
  onClose,
  onConfirm,
  ...
  onClose: () => void;
  onConfirm: (payload: {
    report_reason_code: TCode;
    report_reason_detail?: string;
  }) => void;
  reasons: readonly ReportReasonOption[];
  title: string;
}) {
  const [reasonCode, setReasonCode] = useState<TCode>(reasons[0].code as TCode);
  ...
  function handleClose() {
    setReasonCode(reasons[0].code as TCode);
    setReasonDetail("");
    onClose();
  }
      ...
      <div className="flex min-h-full items-center justify-center">
        ...
        >
          <div className="min-h-0 flex-1 overflow-y-auto p-6">
              ...
              {reasons.map((reason) => (
                ...
                >
                  <input
                    ...
                    name="report-reason"
                    onChange={() => setReasonCode(reason.code as TCode)}
                    type="radio"
                    value={reason.code}
                  />
            ...
            {needsDetail ? (
              <textarea
                className="mt-4 min-h-28 w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-900 outline-none ring-amber-500 focus:ring-2"
                disabled={isSubmitting}
          ...
          <div className="flex shrink-0 flex-wrap justify-end gap-3 border-t border-slate-100 bg-white px-6 py-4">
            ...
            <button
              className="rounded-2xl bg-amber-500 px-5 py-3 text-sm font-black text-white disabled:cursor-not-allowed disabled:bg-slate-300"
