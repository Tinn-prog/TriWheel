...
function formatFare(fare: number | null) {
  return fare !== null ? `PHP ${fare.toFixed(2)}` : "Not set";
}
...
export default function DriverHistoryPage() {
  ...
  async function handleHistoryVisibility(action: "hide" | "unhide") {
    ...
    try {
      const response = await fetch(apiRoutes.driverRideHistory, {
        method: "PATCH",
        headers: {
  ...
  if (isChecking || !user || (!overview && !error)) {
    return (
      <TriWheelLoadingScreen
        message="Loading your completed trips, fares, and passenger details."
          ...
          <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm leading-6 text-orange-50 sm:mt-3 sm:text-base">
                Review your completed and cancelled trips in one dedicated page.
              </p>
            </div>
            <Link
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <p className="text-sm font-bold text-slate-800">
                        Ride #{ride.id}
                        {ride.is_emergency ? (
                          <span className="ml-1.5 rounded-full bg-red-100 px-1.5 py-0.5 text-[10px] font-black uppercase text-red-700">
                            Emergency
                          </span>
                        ) : null}
                      ...
                      <p className="mt-0.5 truncate text-xs text-slate-500">
                        {formatFare(ride.fare)} · {ride.passenger_name ?? "Passenger"} ·{" "}
                    ...
                    <div className="mt-2 space-y-1 border-t border-slate-200/80 pt-2">
                      ...
                      {!ride.driver_rated && (
                        <RideRatingForm
                          ...
                          onSubmit={(event) => handleRatePassenger(event, ride.id)}
                          variant={rideRatingVariant(Boolean(ride.is_emergency))}
                        />
                      )}
                    </div>
